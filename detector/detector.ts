...
export class OpenaiDetector {
  ...
  constructor(private token: string) {
    ...
    this.possible_classes = ['very unlikely', 'unlikely', 'unclear if it is', 'possibly', 'likely'];
  ...
  public detect(text: string, all_probs = false): any {
    ...
    return axios.post('https://api.openai.com/v1/completions', data, {headers: this.header})
      .then((response: AxiosResponse) => {
        if (response.status === 200) {
          const choices = response.data.choices[0];
          const logprobs = choices.logprobs.top_logprobs[0]; 
          const probs = Object.entries(logprobs).reduce((acc, [key, value]) => ({...acc, [key]: 100 * Math.E ** (value as number)}), {});
          const key_prob = probs['"'];
          
          let class_label: string;
          if (this.class_max[0] < key_prob && key_prob < this.class_max[this.class_max.length - 1]) {
            ...
            class_label = this.possible_classes[this.possible_classes.length - 1];
          }
